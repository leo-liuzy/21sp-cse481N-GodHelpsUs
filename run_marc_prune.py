...
class DataTrainingArguments:
    ...
    language: str = field(
        default="en",
        metadata={"help": "The langauge to be tested"},
    )
    num_head: int = field(
        default=16,
        metadata={"help": "The langauge to be tested"},
    ...
    for layer in LAYERS:
        ...
        for head in heads:
            ...
            if training_args.do_eval:
                ...
                lang_head_specific_folder = os.path.join(training_args.output_dir, data_args.language,
                ...
                for eval_dataset, task in zip(eval_datasets, tasks):
                    ...
                    output_eval_file = os.path.join(lang_head_specific_folder, f"eval_results_{task}.txt")
                    if trainer.is_world_process_zero():
                        with open(output_eval_file, "w") as writer:
                            ...
                            for key, value in sorted(eval_result.items()):
                                logger.info(f"  {key} = {value}")
                                writer.write(f"{key} = {value}\n")
                    output_eval_file = os.path.join(lang_head_specific_folder, f"eval_results_{task}.json")
